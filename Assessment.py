...
class Assessment:
    ...
    def format_key_metrics(self, ):
        try:
            #verifying if input json exists
        ...
        #The metrics list would prove efficient if new key metrics are added
        #collecting all key metrics along with their type for processing
        metrics=[]
        ...
        for val in json_data['report']['data']:
            ...
            #iterating on key metrics available and converting them depending on type.
    ...
    def format_products(self, ):
        ...
        #recursive call to this function to fetch hierarchy in the json input
        def flatten(data, name=''):
            try:
                if type(data) is list:
                    i = 0
                    for val in data:
                        # verifying the hierarchy if exists and appending the names with :: placeholder
                        if 'breakdown' in val:
                            ...
                            #inner most hierarchy. collecting data.
                ...
                self.logger.error('Problem processing data. '+str(ex)+traceback.format_exc())
        ...
        #collecting all key elements present to make it dynamic
...
if __name__=='__main__':
    obj = Assessment()
    logging.info("If no parameter is sent, then by default task-1 is executed")
    if len(sys.argv)<2:
        ...
        logging.error("Incorrect parameter: "+sys.argv[1])
